...
<details>
  <summary>hook-form vanilla javascript design</summary>

![form design draft](./images/30/Untitled3.png)
...
### Introduction to react-hook-form and its advantages

A library that makes building forms in React easy. As stated in the [official docs](https://react-hook-form.com/), it is performant, flexible, and excellent at validation.

### Advantages

- less code, easier inputs
- don’t deal with events
- better validation
  - Controlled at the JavaScript layer rather than through HTML attributes
    - Can defend against malicious attacks (if HTML attributes are modified, the browser fails to recognize them: required, minLength, etc.)
- better error
- have control over inputs
- Supports TS out of the box
- Uses functional components (Hook API)
...
- This function lets you register an input/select `Ref` and apply validation through React Hook Form.
...
- When you cannot access a custom component and its `Ref`, you can register the input manually. This can actually be used when working with custom components such as React Native or [react-select](https://github.com/JedWatson/react-select). It is also used to handle this work on your behalf.
...
<details>
  <summary>less code, easier inputs</summary>
...
<details>
  <summary>don’t deal with events</summary>
...
<details>
  <summary>better error (better error handling: set, clear)</summary>
...
<details>
  <summary>have control easier inputs event (easier event control)</summary>
...
- Instead of writing an event handler for every state update, it uses a ref to pull form values from the DOM and process them.
- **PULL** the value
...
- An approach that turns the information currently entered into an HTML element into state, and updates the HTML element's value based on that changed state.
- **PUSH** the value
- The material ui library sometimes passes a Controlled Component that does not control the ref attribute.
...
<details>
  <summary>register vs controller usage example</summary>

- register vs controller usage example
  Reference page: [https://react-hook-form.com/kr/advanced-usage](https://react-hook-form.com/kr/advanced-usage)
